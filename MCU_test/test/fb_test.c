...
void draw_image(int w,int h,void* image,void* finger)
{
	...
	//char p[720*1024]={0};
	char *p = (char *)malloc(h*3*w*3);
    char *q = (char *)malloc(h*3*w*3);
	//char *p=(char*)image;
	//printf("w=%d,h=%d\n",w,h);
	Navi_matResize(image,h,w,p,h*3,w*3);
    Navi_matResize(finger,h,w,q,h*3,w*3);
	h=3*h;
	w=3*w;
	printf("w=%d,h=%d\n",w*3,h*3);
    for (j = 0; j < h; j++)
    ...
    //     	for (i = 0; i < w; i++){
	// 	pix=green32;
    //             loc32[i + j*(stride)+320*(stride)] = pix;
