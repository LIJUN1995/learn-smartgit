...
{
	int m_fd = -1,count = 0,num = 0,i,ret;
	cdfinger_spi_data data;
	struct timeval tp,te;
	char file_name[64] = {0};

	if(argc != 2){
		printf("the number of parameter is errored.  please input two parameters!\n");
		return -1;
	}
	...
	unsigned char *rx = (unsigned char *)malloc(240*240);
	memset(rx,0,240*240);

	data.tx = (unsigned char *)malloc(BUF_SIZE_T);
	data.rx = (unsigned char *)malloc(atoi(argv[1]));
	memset(data.tx,0x90,BUF_SIZE_T);
	memset(data.rx,0x66,atoi(argv[1]));
