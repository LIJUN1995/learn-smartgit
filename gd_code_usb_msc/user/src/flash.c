...
void FingerTech_FlashConfig(void)
{
    chip_cfg temp_cfg={0};
		...
		camera_id = cdfinger_camera_read_id();
		if(camera_id != camera_cfg.camera_id){
			return;
		}
		
		fmc_read_32bit_data(FLASH_USER_START_ADDR, size_w, (uint32_t *)&temp_cfg);
    if(camera_cfg.update == 1 || temp_cfg.stat != 0x55){
			update_camera_cfg();
    }else{
			camera_cfg = temp_cfg;
		}
...
void update_camera_cfg(void){ 
    fmc_erase_sector(CTL_SECTOR_NUMBER_6);
    ob_write_protection0_disable(OB_WP_6);
